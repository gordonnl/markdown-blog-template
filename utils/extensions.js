...
const customId = {
  ...
  tokenizer(src, tokens) {
    ...
    if (match) {
      return {                                         // Token to generate
        type: 'customId',                              // Should match "name" above
        raw: match[0],                                 // Text to consume from the source
        caption: this.lexer.inlineTokens(match[1].trim())   // Additional custom properties, including
      };
    }
  ...
  renderer(token) {
    return `<div id="${this.parser.parseInline(token.caption)}"></div>`;
  },
  childtokens: ['linkId'],                 // Any child tokens to be visited by walkTokens
};
  
module.exports = { imageCaption, customId };
